    ...
    <div className="flex flex-col h-screen bg-white font-sans overflow-hidden">
      ...
      <header className="flex-none border-b bg-white px-6 py-3 flex items-center justify-between shadow-sm z-[1001]">
        <div className="flex items-center gap-10 flex-1">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-2.5 border border-gray-200 rounded-full bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
              value={searchQuery}
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/mypage")} className="flex items-center gap-2 p-2 px-4 bg-gray-50 rounded-full font-bold text-sm border border-gray-100 hover:bg-indigo-50 transition-colors">
            <User className="h-4 w-4 text-indigo-600" /> 마이페이지
          </button>
          <button onClick={handleLogout} className="p-2 px-4 text-sm font-bold text-red-500 hover:bg-red-50 rounded-full">로그아웃</button>
        </div>
      </header>
                  ...
                  {openPlaceId === place.externalId && (
                    <CustomOverlayMap position={{ lat: place.latitude, lng: place.longitude }} yAnchor={1.35}>
                      <div className="bg-white rounded-[28px] shadow-2xl border border-gray-100 p-5 min-w-[240px] relative">
                        ...
                        <strong className="text-base font-black block mb-1 truncate pr-6 text-gray-900">{place.name}</strong>
                        <div className="flex items-center gap-1 text-amber-500 text-xs font-black mb-4">
                           <Star className="h-3 w-3 fill-current" />
                           {place.averageRating?.toFixed(1) || "0.0"}
                           <span className="text-gray-300 ml-1 font-bold">({place.reviewCount || 0})</span>
                        </div>
                        <button
                          onClick={() => goToDetail(place)}
                          className="w-full py-3 bg-indigo-600 text-white rounded-2xl text-xs font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
                        >
                          상세보기
                        </button>
